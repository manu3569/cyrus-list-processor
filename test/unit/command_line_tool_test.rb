...
class TestCommandLineTool < Minitest::Test
  ...
  def test_help_output
    ...
    assert_match /Usage:/, command_line_tool.result
  end

  def test_missing_list_message
    command_line_tool = CommandLineTool.new(["--output=1"])
    assert_match /You must specify at least one record file/, command_line_tool.result
  end

  def test_missing_sort_order
    command_line_tool = CommandLineTool.new(["--pipe=test/data/pipe.txt"])
    assert_match /You must specify a sort order/, command_line_tool.result
  end
  ...
  def test_processing_lists_with_result_ordered_by_gender_and_last_name
    ...
    assert_match gender_order_regex, command_line_tool.result
  end
  ...
  def test_processing_lists_with_result_ordered_by_date_of_birth
    ...
    assert_match date_of_birth_order_regex, command_line_tool.result
  end
  ...
  def test_processing_lists_with_result_ordered_by_last_name_descending
    ...
    assert_match last_name_order_regex, command_line_tool.result
  end
